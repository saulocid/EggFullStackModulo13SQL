...
select codigo_oficina as "Código de Oficina", ciudad as "Ciudad" from oficina;
# 2. Devuelve un listado con la ciudad y el teléfono de las oficinas de España.
select ciudad as "Ciudad", telefono as "Teléfono" from oficina where pais = "España";
# 3. Devuelve un listado con el nombre, apellidos y email de los empleados cuyo jefe tiene un código de jefe igual a 7.
select nombre as "Nombre", apellido1 as "Apellido", apellido2 as "Apellido", email as "e-mail" from empleado where codigo_jefe = 7;
# 4. Devuelve el nombre del puesto, nombre, apellidos y email del jefe de la empresa.
select puesto as "Puesto", nombre as "Nombre", apellido1 as "Apellido", apellido2 as "Apellido", email as "e-mail" from empleado where codigo_jefe is null;
# 5. Devuelve un listado con el nombre, apellidos y puesto de aquellos empleados que no sean representantes de ventas.
select nombre as "Nombre", apellido1 as "Apellido", apellido2 as "Apellido", puesto as "Puesto" from empleado where puesto <> "Representante Ventas";
# 6. Devuelve un listado con el nombre de los todos los clientes españoles.
select nombre_cliente as "Nombre Clientes" from cliente where pais = "Spain";
# 7. Devuelve un listado con los distintos estados por los que puede pasar un pedido.
select estado as "Estados del Pedido" from pedido group by estado;
# 8. Devuelve un listado con el código de cliente de aquellos clientes que realizaron algún pago
# en 2008. Tenga en cuenta que deberá eliminar aquellos códigos de cliente que aparezcan repetidos. Resuelva la consulta:
# 8.1 usando YEAR
select distinct codigo_cliente as "Código de Cliente" from pago where YEAR(fecha_pago) = 2008 order by codigo_cliente asc;
# 8.2 usando DATE_FORMAT
select distinct codigo_cliente as "Código de Cliente" from pago where date_format(fecha_pago, '%Y') = 2008 order by codigo_cliente asc;
# 8.3 sin funciones
select distinct codigo_cliente as "Código de Cliente" from pago where fecha_pago like "%2008%" order by codigo_cliente asc;
# 9. Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de
# entrega de los pedidos que no han sido entregados a tiempo.
SELECT 
    codigo_pedido AS 'Código Pedido',
    codigo_cliente AS 'Código Cliente',
    fecha_esperada AS 'Fecha Esperada',
...
select * from pedido where YEAR(fecha_pedido) = 2009 and estado = "Rechazado" order by codigo_pedido asc;
# 12. Devuelve un listado de todos los pedidos que han sido entregados en el mes de enero de cualquier año.
select * from pedido where MONTH(fecha_entrega) = 01 and estado = "Entregado" order by codigo_pedido asc;
...
select distinct forma_pago as "Pago" from pago;
...
select distinct c.nombre_cliente as "Cliente", e.nombre as "Representante", o.ciudad as "Ciudad"
	from cliente c 
    inner join pago p 
    on c.codigo_cliente = p.codigo_cliente
    inner join empleado e 
...
select o.linea_direccion1 as "Dirección", 
...
# 7. Devuelve el nombre de los clientes y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el 
# representante.
SELECT c.nombre_cliente AS 'Nombre Cliente',
    e.nombre AS 'Representante',
    o.ciudad as 'Ciudad Representante'
	FROM cliente c
	INNER JOIN empleado e 
    ON e.codigo_empleado = c.codigo_empleado_rep_ventas
    inner join oficina o 
...
select distinct pro.gama as "Gama"
	from producto pro
    inner join detalle_pedido dp
    on pro.codigo_producto = dp.codigo_producto
    inner join pedido p 
